...
public class PNGSpriteRendererComponent extends SimpleAbstractAnimationComponent{
	...
	public void onRender() {
		...
		ip.getPicture().draw(getPos().x, getPos().y, getPos().x + tileX, getPos().y + tileY, tileX * xOffset, tileY * state, tileX * (xOffset + 1), tileY * (state +1));
//		ip.getPicture().draw(getPos().x, getPos().y, tileX * xOffset, tileY * state, tileX * (xOffset + 1), tileY * (state +1));
	}
